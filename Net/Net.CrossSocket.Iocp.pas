...
procedure TIocpCrossSocket.StopLoop;
  ...
  procedure _CloseHandle(AIocpHandle: THandle; APerIoDataCount: NativeInt);
  begin
    TTask.Run(
      procedure()
      var
        ...
        LPerIoData: PPerIoData;
      begin
        while APerIoDataCount > 0 do
        begin
          GetQueuedCompletionStatus(AIocpHandle, LBytes, ULONG_PTR(LSocket), POverlapped(LPerIoData), INFINITE);
          if LPerIoData <> nil then
          begin
            LPerIoData.CrossData := nil;
            LPerIoData.Callback := nil;
            FreeMem(LPerIoData, SizeOf(TPerIoData));
            Dec(APerIoDataCount);
          end;
        end;
        CloseHandle(AIocpHandle);
      end
    );
  end;
...
begin
  ...
  _CloseHandle(FIocpHandle, FPerIoDataCount);
end;
